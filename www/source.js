...
{
  var data = $("#jqxgrid").jqxGrid( 'getrowdata', row );
  return '<div style="overflow: hidden; text-overflow: ellipsis; padding-bottom: 2px; text-align: left; margin: 4px;">' + '<a href="tvd?c=transponder&a=show&source_id=' + source_id + '&transponder_id=' + data["id"] + '">' + data["name"] + '</a>' + '</div>';
}
  ...
  $("#jqxgrid").jqxGrid(
    {
      ...
      columnsresize: true,
      columns: [
        { text: 'Transponder', datafield: 'name', width: 400, cellsrenderer: renderLink },
        { text: 'Type', datafield: 'type', width: 170, cellsrenderer: renderType },
        { text: 'State', datafield: 'state', width: 170 },
